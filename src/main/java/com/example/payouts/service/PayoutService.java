...
public class PayoutService {
  ...
  public PayoutCreationResult createPayout(
      PayoutRequest request, String idempotencyKey, String clientId) {
    Optional<Payout> existing = findExistingPayout(clientId, idempotencyKey);
    return existing
        .map(payout -> new PayoutCreationResult(payout, false))
        .orElseGet(() -> attemptSave(request, idempotencyKey, clientId));
  }

  private PayoutCreationResult attemptSave(
      PayoutRequest request, String idempotencyKey, String clientId) {
  ...
  private Optional<Payout> findExistingPayout(String clientId, String idempotencyKey) {
    Optional<Payout> existing =
        payoutRepository.findByClientIdAndIdempotencyKey(clientId, idempotencyKey);
        ...
        .orElseThrow(
            () ->
                new IllegalStateException(
                    "Payout should exist but was not found after constraint violation. Key: "
                        + idempotencyKey));
  }
